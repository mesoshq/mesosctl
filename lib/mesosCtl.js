...
module.exports = function () {
    ...
    var mesosCtl = {
        ...
        config: {},
        functions: {
            loadPackages: function  (repositoryData) {

                repositoryData.packages.forEach(function (pkg){

                    // Index package in lunr for package search
                    mesosCtl.packages.index.add({
                        name: pkg.name,
                    ...
                    // Add package to list
                    mesosCtl.packages.list.push(pkg.name);

                    // Add to map
                    mesosCtl.packages.map[pkg.name] = pkg;

                });

            },
            initPackageIndex: function () {
                if (mesosCtl.functions.checkRepository()) {

                    // Reset other properties
                    mesosCtl.packages.list.length = 0;
                    mesosCtl.packages.map = {};
                    mesosCtl.packages.loaded = false;

                    // Load local package index
                    mesosCtl.functions.loadPackages(mesosCtl.functions.getLocalRepositoryIndex());

                    // Mark as loaded
                    mesosCtl.packages.loaded = true;
            ...
            ensureConfigStoragePath: function () {
                fs.stat(mesosCtl.options.configStoragePath, function (err, stat) {
                    if (err) {
